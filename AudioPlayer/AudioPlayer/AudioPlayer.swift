...
public protocol AudioPlayerDelegate: NSObjectProtocol {
     ...
     */
    func audioPlayer(_ audioPlayer: AudioPlayer, didFinishPlayingItem item: AudioItem)
     ...
     */
    func headers(_ URL: URL) -> [String: String]
}
...
open class AudioPlayer: NSObject {
    ...
    fileprivate let reachability = Reachability.reachabilityForInternetConnection()
    ...
    open fileprivate(set) var currentItem: AudioItem? {
        didSet {
            ...
            if let currentItem = currentItem {
                ...
                player = AVPlayer(playerItem: playerItemWithHeaders(URLInfo.URL as URL))
    ...
    open var clearQueueAfterFinish = true
    ...
    */
    open func addItemsToQueue(_ items: [AudioItem]) {
        if currentItem != nil {
            ...
            var toAdd = items.map { (audioItem: (AudioItem)) -> (position: Int, item: (AudioItem)) in
                let item = (position: idx, item: audioItem)
                idx += 1
                return item
            }
    ...
    */
    open func seekToTime(_ time: TimeInterval, toleranceBefore: CMTime = kCMTimePositiveInfinity, toleranceAfter: CMTime = kCMTimePositiveInfinity) {
     ...
     */
    open func adjustQuality(_ quality: AudioQuality) {
        if let currentQuality = currentQuality, let currentItem = self.currentItem {
            let URLInfo: AudioItemURL = {
                switch quality {
            ...
            if URLInfo.quality != currentQuality {
                ...
                let item = playerItemWithHeaders(URLInfo.URL as URL)
    ...
    */
    fileprivate func playerItemWithHeaders(_ URL: Foundation.URL) -> AVPlayerItem {
